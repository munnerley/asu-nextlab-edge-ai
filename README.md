...
### Frontend changes
Frontend updates are fully automated. When a change is published in Lovable:
...
### Open WebUI config changes (admin settings, models, knowledge base)
After making changes in the Admin WebUI:
1. Double-click `update-webui.bat`
2. Enter the new version number when prompted

This will automatically:
- Export the Open WebUI config from Docker
...
| Version | Notes |
|---------|-------|
| v1.0.9  | Automated Lovable frontend sync via GitHub Actions — frontend updates and version bumps happen automatically on every Lovable publish. Renamed export-config.bat to update-webui.bat |
